...
namespace RDPro.ViewModels
{
    public class AllConnectionsViewModel : ReactiveObject
    {
        ...
        public ICommand ImportConnectionsCommand { get; }
        ...
        public ICommand ExportRdpXFileCommand { get; } = ReactiveCommand.CreateFromTask<Models.ConnectionDetails>(async (details) =>
        {
            var title = "Export .RDPX File";
            var message = "This feature is not implemented yet. Please check back in a future update.";
        ...
        public AllConnectionsViewModel()
        {
            ...
            ImportConnectionsCommand = ReactiveCommand.Create(async () =>
            {
                var title = "Import Connections (Beta)";
                var message = "This Feature is currently in beta, please be cautious when using it. \n\nThis will import .rdpx files into RD Pro. Existing connections with the same Connection Name will be overwritten. \n\nIt is recommended to back up your existing connections before proceeding.";
                bool confirmed = await UiService.ShowConfirmAsync(title, message);
                if (confirmed)
                {
                    Console.WriteLine("User confirmed import of connections.");
                    ...
                    {
                        Title = "Select RDPX Files to Import",
                        AllowMultiple = true,
                        FileTypeFilter = new[]
                        {
                            new FilePickerFileType("RDPX Files") { Patterns = new[] { "*.rdpx" } },
                    ...
                    if (files.Any())
                    {
                        string destinationFolder = ConnectionFileService.GetEnhancedPath();
                        int importCount = 0;
                        int errorCount = 0;

                        foreach (var sourcePath in files.Select(f => f.Path.LocalPath).Where(p => p != null))
                        {
                            try
                            {
                                string fileName = Path.GetFileName(sourcePath);
                                string destinationPath = Path.Combine(destinationFolder, fileName);
                                File.Copy(sourcePath, destinationPath, true); // Overwrite existing

                                // Load the new connection and add/update it in the main list
                                var importedConnection = ConnectionFileService.Load(destinationPath);
                                if (importedConnection != null)
                                {
                                    var existing = _allConnections.FirstOrDefault(c => c.ConnectionID == importedConnection.ConnectionID);
                                    if (existing != null) _allConnections.Remove(existing); // Remove old one
                                    _allConnections.Add(importedConnection); // Add new one
                                    importCount++;
                                }
                            }
                            catch (Exception ex)
                            {
                                errorCount++;
                                Console.WriteLine($"Failed to import file {sourcePath ?? "N/A"}: {ex.Message}");
                            }
                        }
                        await UiService.ShowInfoAsync("Import Complete", $"Successfully imported {importCount} connection(s).\nFailed to import {errorCount} file(s).");
                    }
                }
            });
