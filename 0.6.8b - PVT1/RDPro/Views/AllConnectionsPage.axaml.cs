    ...
    public partial class AllConnectionsPage : UserControl
    {
        ...
        private bool ShowProgrammaticMenu(Avalonia.Controls.Control placementTarget, ConnectionDetails details, AllConnectionsViewModel vm)
        {
            try
            {
                ...
                list.Add(new MenuItem { Header = "Export .RDPX File", Command = vm.ExportRdpXFileCommand, CommandParameter = details });
                list.Add(new Separator());
        ...
        private void ShowActionsPopup(Avalonia.Controls.Control? placementTarget, ConnectionDetails details, AllConnectionsViewModel vm)
        {
            try
            {
                ...
                panel.Children.Add(MakeButton("Export .RDPX File", vm.ExportRdpXFileCommand));
                AddSeparator();
                // Favourite toggle
        ...
        private void importconnectionscommand(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            try
            {
                var vm = this.DataContext as AllConnectionsViewModel;
                if (vm != null && vm.ImportConnectionsCommand != null && vm.ImportConnectionsCommand.CanExecute(null))
                {
                    vm.ImportConnectionsCommand.Execute(null);
                }
            }
            catch (Exception ex)
            {
                try { Console.WriteLine($"ImportConnectionsCommand error: {ex.Message}"); } catch { }
            }
