...
namespace RDPro.Views
{
    ...
    private void Item_ContextRequested(object? sender, ContextRequestedEventArgs e)
        ...
        private void ShowActionsPopup(Avalonia.Controls.Control? placementTarget, ConnectionDetails details, AllConnectionsViewModel vm)
        {
            try
            {
                ...
                Button MakeButton(string text, ICommand command)
                {
                    ...
                    b.Click += async (_, __) =>
                    {
                        try
                        {
                            ...
                            // If this is the delete command, show confirmation first on the UI thread.
                            if (command == vm.DeleteConnectionCommand)
                            {
                                bool ok = await RDPro.Services.UiService.ShowConfirmAsync("Delete Connection", $"Delete connection '{details.ConnectionName}'? This cannot be undone.");
                                if (ok && command.CanExecute(details)) command.Execute(details);
                            }
                            else
                            {
                ...
                AddSeparator();
                panel.Children.Add(MakeButton("Delete", vm.DeleteConnectionCommand));
    ...
    private void Item_ActionsButtonClick(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        ...
        // Handler for the ContextMenu Delete menu item defined in XAML; shows confirm then executes.
        private async void ContextMenu_Delete_Click(object? sender, Avalonia.Interactivity.RoutedEventArgs e)
        {
            try
            {
                // The MenuItem's DataContext is the ConnectionDetails from the DataTemplate
                if (sender is MenuItem menu && menu.DataContext is ConnectionDetails details)
                {
                    var vm = this.DataContext as AllConnectionsViewModel;
                    if (vm != null)
                    {
            ...
            {
                try { Console.WriteLine($"ContextMenu_Delete_Click error: {ex.Message}"); } catch { }
            }
        }
    }
}
